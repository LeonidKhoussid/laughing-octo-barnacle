...
| --- | --- | --- | --- |
| [S1] | Исходное ТЗ в сообщении пользователя | Отражено в мастер-промпте | Полный контракт позднее получен в [S8] и [S11] |
...
| [S9] | Экспорт Telegram, преимущественно «Вопросы по задаче» | `/Users/leo/Downloads/ChatExport_2026-09-23 (2).zip`, внутри `ChatExport_2026-09-23/messages.html` | Прочитан 23.09.2026; сообщения организатора и мнения участников разделены в `docs/chat-review-2026-09-23.md`; BERT в runtime разрешён, типичная нагрузка с разгоном ~330/пики 1000, 429 учитывается отдельно |
| [S10] | Предложенный пользователем датасет `wolframko/russian-pii-66k` | https://huggingface.co/datasets/wolframko/russian-pii-66k/tree/d458b5a2e299d2e16adbd3b3921487f652b06a96 | Полный структурный аудит 65 652 строк и 210 647 аннотаций; `docs/russian-pii-66k-audit.md`. Лицензия и происхождение в карточке не указаны; разметки public/private нет. Источник кандидата для обучения, не требование организаторов и не доказательство качества модели |

| [S11] | `Модуль_безопасности_ПД (1).pdf`, 9 страниц | `/Users/leo/Downloads/Модуль_безопасности_ПД (1).pdf` | Прочитан 23.09.2026: §7 требует исходный ZIP и доступный сервис; Приложение A задаёт `/process`, Приложение B — 10 с timeout, ретраи, 429, ~5 минут и 1000 RPS. Локальные тесты не являются результатом скрытого тестера |
