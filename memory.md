...
## 20. Независимая проверка опубликованного сервиса и аудит датасета — 23.09.2026

- По запросу пользователя два отдельных субагента GPT-5.6 Terra проверили опубликованный сервис и датасет; GPT-6 Astra для этих подзадач не использовался. GPT-6 Terra не был доступен среди моделей инструмента.
- Опубликованный `http://201.34.146.180:8000/`: 24 последовательных HTTP-запроса на синтетических данных, 18/18 ограниченных функциональных проверок прошли. Проверены доступность, маскирование/повторы/точное восстановление, 409 при конфликте, выбранные типы ПД, публичный и частный контекст, Unicode, названия произведений, текст без ПД и безопасный 422. Отдельно в фоновом in-app браузере наблюдались три скрытых фрагмента и точное восстановление (245 мс UI, не нагрузочная метрика). Отчёт: `artifacts/deployed-review-terra-2026-09-23.{json,md}`.
- Во время проверки появился frontend asset `index-DAHPviHP.js`, совпадающий по имени с текущей локальной сборкой; ранее root наблюдал `index-CBGDl99e.js`. Это не подтверждает Git-ревизию backend. Action A без credentials ответил 401; причина/политика и новое поле `decisions` этим запросом не проверены. Нагрузка, SLA, TTL, multiworker, конфигурация сервера и полное покрытие ПД в этом проходе НЕ проверялись. Известные диагностические ошибки раздела 19 остаются; успешный smoke их не отменяет.
- Датасет [S10], revision `d458b5a2e299d2e16adbd3b3921487f652b06a96`: полный локальный scan 65 652 строк / 210 647 spans, 17 меток. Все значения совпадают с указанными интервалами в Python-индексации; ошибок границ, пересечений и точных дубликатов текстов не обнаружено. Нет символов вне BMP, поэтому этот корпус не позволяет отличить code-point от UTF-16 convention на таких символах. После замены проверенных spans метками найдены 307 повторяющихся шаблонов / 470 повторных строк сверх первой. Структурная корректность не подтверждает смысловую полноту разметки.
- В корпусе нет документов с пустой PII-разметкой и нет отдельных public/private меток; карточка содержит только YAML, лицензия/происхождение не заявлены. Он пригоден для рассмотрения как источник обучения entity detection, но сам по себе не обучает нужное решение о публичности. Текущий semantic adapter принимает только PER из NER; новая таксономия требует явного сопоставления. Аудит: `artifacts/russian-pii-66k-audit.json`, `docs/russian-pii-66k-audit.md`.
- Production-код, активные модели и пороги в этом проходе не менялись. Обучение, экспорт, повторный benchmark и оптимизация NOT_RUN; пользователь отложил оптимизацию до проверки. Набор анализировался локально, строки датасета не отправлялись опубликованному сервису. Следующий обоснованный эксперимент — отдельный NER-кандидат с проверенной таксономией и группированным holdout; для контекстного классификатора нужны отдельные размеченные публичные/частные/неоднозначные упоминания. Прежний запрет повторять падающий macOS Torch-экспорт сохраняется.


## 21. Проверка полного ТЗ, §7 и Приложений A/B — 23.09.2026

- По прямому запросу пользователя исправлены локальные исходники; сервер пользователя не обновлялся, SSH-доступ не предоставлен. Параметры сервера со слов пользователя: 4 × 3,3 ГГц CPU, 8 GB RAM, 80 GB NVMe. Для воспроизводимого запуска подготовлен Docker с React production build и отдельным кэшируемым слоем SHA-256-проверенных моделей; базовый judge flow не требует `.env` или API-ключа.
- Все 17 типов проверяются 19 замороженными примерами через настоящий model-enabled `/process`, с проверкой категории, маски и точного восстановления. Исправлены межстрочные границы CARDHOLDER_NAME, варианты дат, порядок адресных компонентов, склонения ФИО, частный контекст с соседним email и публичная научная/литературная атрибуция. Диагностические неоднозначные случаи не объявлены решёнными, эталоны не переписаны.
- Реализован настоящий 9-секундный deadline, включая очередь, для `/process` и основных demo endpoints. Перегрузка `/process` возвращает 429 + Retry-After. Работающий нативный вызов после timeout сохраняет слот до завершения, а отменённая очередь не запускает callback позже. Полный JSON ограничивается до парсинга (включая неизвестные поля/chunked body), декодированный текст ограничен отдельно.
- Логи отражают выполненные vault/detect/resolve/mask/unmask/result/errors и типы без исходного текста, payload ID, токенов и секретов. Исправлены безопасные validation/config ошибки и однократный учёт запросов. Метрики различают оценку входных токенов и фактические токены успешного NER/NLI inference; RPS/TPS имеют ограниченное 60-секундное окно и относятся к одному worker.
- Пересчитан размер long-input: 100 082 токена активного NER-токенизатора, 659 119 символов, 1 223 987 UTF-8 байт. Прежний chars/4-пример содержал только 60 897 таких токенов. Лимит текста поднят до 2 MiB. Новые тесты проверяют ПД в начале/середине/конце и полное восстановление. NER batch=8 сохранён: смена batch меняла интервалы. Четыре NER intra-op потока сократили отдельный Mac NER-прогон 6,778 → 2,427 с; NLI остаётся однопоточным. Локальная memoization ролей имён сократила detector-only 1,159 → 0,912 с без сохранения ПД между запросами. Это не HTTP SLA.
- PASSED: `.venv/bin/pytest --junitxml=artifacts/requirements-full-tests.xml` — **584 passed, 0 skipped**, 28,80 с, exit 0, 7 deprecation warnings; включены временный настоящий Redis, нативные модели, 100k и дедлайны. Отчёт: `artifacts/requirements-full-tests.{log,xml}`. Docker production build и health ready=true прошли. В фоновом браузере проверены обычный клиент (3 маски), литературный Пушкин (0 масок), клиент-тёзка (3 маски), точное восстановление, визуальный результат и отсутствие console errors. Dev frontend5173/backend8000 перезапущены.
- Короткий Linux/arm64 Docker-тест с квотой 4 CPU / 6 GiB памяти, closed concurrency=8, required/public/mixed и restoration: 281/281 правильных HTTP 200, 273 внутри 20 с, **13,65 RPS**, p95 **2719,22 мс**, p99 2890,86 мс. Это НЕ эквивалент измерению на сервере пользователя и НЕ прохождение 1000 RPS / 1 с. NER=1 дал те же 13,65 RPS. Временный NER-only no-spin дал 14,50 RPS в одном прогоне, без улучшения mask p95; в production не принят. Модели/пороги не ослаблялись ради скорости.
- Tiny-NLI кандидат в отдельном ONNX-only эксперименте не принят: консервативный cascade не принял ни одной из 67 задач и добавлял накладные расходы. Во время эксперимента менялся исходный detector-код, поэтому его отчёт также не является сравнением финальной версии. Torch/export на этом Mac не запускался.
- Полная матрица и ограничения: `docs/requirements-verification.md`; серверные действия: `docs/server-deployment.md`; актуальные нагрузочные числа: `docs/benchmark.md`. 95% на репрезентативном независимом наборе, 1000 успешных RPS, 1 с на 100k и банковское нормативное соответствие не подтверждены.

- Продлённая проверка первого контейнера (до исправления индекса, limit=64) НЕ ПРОШЛА: за 300 с при target=1000/200 connections реально запущено 25 018 HTTP, 22 661 × 429, 878 timeout, 3 transport error; из 1476 HTTP 200 — 29 неправильных масок. Короткий тест этих ошибок не выявил. Контейнер не упал, OOM events=0. Семь записанных ошибок ADDRESS из отдельного 30-секундного воспроизведения повторились без нагрузки: индексный regex скрывал шестизначную часть внутри буквенно-цифрового идентификатора. Исправлены Unicode-границы индекса, добавлены три регрессии. Все шесть настоящих частей адреса были скрыты; первоначальный вывод о пропуске коротких значений был отозван (цифра может входить в сам случайный токен). Исходные строки/эталоны генератора не изменены.
- Отчёт генератора теперь отдельно сохраняет до 10 безопасных примеров каждого вида ошибки, чтобы первые 429 не вытесняли ошибки правильности. Добавлена регрессия. Значения ПД и тела ответов в отчёт не записываются. Старый отчёт с 29 ошибками не переписан как успешный.
- Конфигурационный 30-секундный прогон с max_in_flight=8 на прежнем образе: 189 правильных ответов (181 в окне, 6,03 RPS), 3028 × 429, 6 transport errors, без timeout. Прежний 30-секундный прогон limit64 имел 107 timeout. Значение по умолчанию снижено до 8 во всех launch paths, чтобы ограничить очередь дорогой модели; это не достижение SLA. После исправлений полный pytest: 584 passed, exit 0.

- После исправлений выполнен чистый Docker build из распакованного исходного ZIP. Проверены отсутствие `.env`/API-ключей, health, React asset, mask/retry/restore/retry, 19/19 требуемых примеров и прежний ошибочный идентификатор адреса. Образ `alfagen-pii-gateway:source-zip-check` имеет ID `sha256:d20175faae43781ec5937d2838d4c0d4a726c7f6b1b51de00b68e365522f4779`; отчёт `artifacts/source-zip-container-check.json`. В браузере production-сборки подтверждены 6 адресных масок и сохранённый идентификатор, точный round-trip.
- На этом образе (4 CPU / 6 GiB) настоящий HTTP `/process` на 100 082 токенах: mask 200 за 8,3486 с, restore 200 за 0,5631 с, начало/середина/конец скрыты, exact_restore=true. Это один синтетический документ без нагрузки, не p95/100k SLA: `artifacts/requirements-container-100k.json`.

- Финальные прогоны исправленного исходного ZIP на том же Docker (4 CPU/6 GiB, concurrency8, свежие ID/text и ready restores): required, 20 с — 9587/9587 правильных ответов, 9579 в окне, 478,95 RPS, общий p95 31,66 мс; required/public/mixed, 30 с — 407/407, 399 в окне, 13,30 RPS, общий p95 2833,56 мс. Ни одного correctness/HTTP/timeout/transport failure в этих закрытых прогонах, accounting reconciled. Они не повторяют прежний open-overload и не подтверждают, что его timeout/transport failures устранены. Метаданные и отчёты: `artifacts/docker-source-zip-final-*`.
- Следующее обязательное действие пользователя: передать актуальный `artifacts/alfagen_source.zip` на сервер, распаковать в каталог проекта и выполнить `docker compose up --build -d`, затем readiness/smoke и измерение на самом сервере по `docs/server-deployment.md`. При существующем `.env` заменить прежний PII_MAX_IN_FLIGHT=64 на 8. Новая версия публично не развёрнута этим проходом. 1000 успешных RPS и репрезентативные 95% остаются открытыми; отдельная задача — более дешёвая проверенная контекстная модель и независимые данные для её калибровки/оценки.
## 22. Изоляция перегрузки по серверному логу — 23.09.2026

- Пользователь предоставил `logs.md`: 816 строк, 33 HTTP 200 и 268 HTTP 429 для `/process`; 507 строк обрезаны pager. Лог указывает systemd unit `alfagen-pii-gateway.service`, поэтому в серверную инструкцию добавлено обновление существующего checkout и restart этого unit, без запуска второго сервиса на 8000. Полные пяти-минутные серверные результаты и причины старых 429 по этому фрагменту не восстанавливаются.
- Исправлена конкуренция за общий admission: при заполнении обычных слотов отдельная ограниченная очередь обслуживает только уже зафиксированные mask replay / restore. `Lifecycle.process_existing` использует прежние namespace, сравнение текста/отпечатка, conflict и TTL; отсутствие/PROCESSING не запускают NLP в резервной очереди. Оба лимита по умолчанию 8. У резервной очереди свой AnyIO limiter, поэтому заполненный обычный thread pool её не блокирует.
- Контекстный NLI ограничен двумя одновременными вызовами на процесс (одним на одноядерном host), освобождая обработчики для NER/коротких запросов. При переполнении — явный 429, без слабого fallback. Модели, thresholds, правила детекции, фикстуры и NLP не отключались/не менялись; ранее отклонённый INT8 не активирован. Кэша предсказаний нет.
- Добавлены безопасные причины overload `admission/context/deadline` в логах и счётчиках. Генератор сохраняет числа правильных mask/restore по видам примеров отдельно от всех попыток. PASSED: 587 tests, 0 skipped, exit 0, 31,31 с; `artifacts/overload-fix-tests.{log,xml}`. Три новых регрессии покрывают starvation, foreign/missing/processing/conflict, модельные слоты и безопасное отображение context overload.
- 30-секундные открытые диагностики (1000 scheduled RPS, concurrency200, required/public/mixed + ready restores): native Mac 44,33 → 119,73 правильных RPS; Linux/arm64 Docker с квотой 4 CPU/6 GiB — 5,80 → 26,37. Docker transport errors 66 → 8, неверных HTTP 200 не было. Генератор не достиг offered rate; состав реально отправленных запросов менялся с доступностью restores. Это улучшение обслуживания смешанного потока под перегрузкой, не ускорение самой модели и не прохождение 1000 RPS.
- Новый 300-секундный Docker-прогон при 330 scheduled RPS: 32739 HTTP попыток, 20126 правильных ответов (20118 в окне, 67,06 RPS), 12574 × 429, 39 transport errors, 0 incorrect 200, 0 timeout; successful p95 269,32 мс. Успешные mask включают все 17 типов, 2173 public и 2126 mixed cases. Пропущены генератором 66099 late slots и 162 client-capacity slots. Health ready=true. Это не passed SLA. Источник: `artifacts/overload-docker-after-5min.json`.
- Полное описание: `docs/overload-repair.md`; серверные шаги: `docs/server-deployment.md`. Публичный сервер не изменялся. Изменения локальные: перед redeploy нужны commit/push либо передача нового source ZIP. Следующий существенный performance шаг — меньшая проверенная модель контекста; нельзя объявлять требование закрытым только увеличением лимита или числом ответов 429.
- Уточнение transport errors: отдельная 30-секундная диагностика зафиксировала 3 RemoteProtocolError. После увеличения только HTTP idle keep-alive с 5 до 30 с новый 30-секундный прогон: 1956 корректных ответов, 1130 × 429, 0 transport/timeout/incorrect, 65,0 правильных RPS в окне, success p95 219,37 мс. Это не доказывает причину всех прежних 39 сбоев. Docker/Compose/.env.example получили UVICORN_TIMEOUT_KEEP_ALIVE=30; для systemd документирован override. Processing deadline остаётся 9 с.
- Чистый Docker build из ZIP прошёл, image sha256:d6bf9197edcb14959e700fcc923105c70dcb9ab19d092117fb90ddb0b59c38f9. No-env/no-key HTTP: 19 обязательных примеров (17 типов), root/React asset, mask/replay/restore/replay прошли. Дополнительный публичный пример про драматурга Бернарда Шоу пере-маскируется; ошибка воспроизведена и на сохранённом baseline до ремонта (FULL_NAME 10..21). Эталон не менялся, ошибка не объявлена исправленной. Дополнительный клиент-тёзка защищён; exact restore прошёл для всех 21 случаев. `artifacts/overload-release-container-check.json`: 20/21 masking, 21/21 restore. Существующий suite 587 passed не включает это новое ошибочное наблюдение.

## 23. Повторный scoring и короткие/длинные NER-запросы — 23.09.2026

- `scoring 2.log` совпадает с прошлым summary: score=9844, 31 issue (2 critical, 26 major, 3 minor). Деталей нет; эти issue не объявлены исправленными. В переданном серверном отчёте 16 748 successes / 320 909 HTTP = 5,22%; 1156 total RPS включают 429. Один read-only GET `/metrics` подтвердил 18 292 successes / 324 900 overloads позже; внешняя нагрузка и deployment не выполнялись.
- Четыре чередующихся прогона на 4 CPU / 6 GiB: NER 4 потока дал structured 437,5/466,2 correct RPS, NER 1 — 632,3/587,6. Mixed: 125,7/117,2 против 240,8/293,8, с 429. Глобальный NER=1 провалил 100 082-токенный HTTP тест (429 за 9,05 с); такой default не принят.
- Короткий NER (<=254 encoded tokens) теперь использует отдельную однопоточную сессию тех же ONNX весов, длинный — прежнюю до четырёх потоков. Модели, tokenizer, thresholds, детекторы и эталоны не менялись. Длинный HTTP повтор: mask 8,54 с, restore 0,60 с, exact round-trip; 1-секундный SLA всё ещё не выполнен.
- Добавлены outcome rates по operation, отдельные latency по outcome и native model batch timings; total RPS/estimated input TPS явно включают отказы. `PII_LOG_FILE` включает ротацию 100 MiB + 5 backup с UTC timestamp; инструкция systemd сохраняет стадии в файл и отключает повторный access log. Проверено 133 655 событий: terminal outcomes совпали со счётчиками (25 196 success, 6911 overload), без sampling. Значения ПД не добавлены в логи.
- Финальный локальный контейнер: structured 15 с 560,93 correct RPS / 8422 successes / 0 errors; mixed 15 с 319,80 / 4803 successes / 1912 HTTP 429; mixed open 60 с target1000 реально 16 959 attempts, 11 960 successes, 4999 HTTP 429, 199,30 correct RPS, без incorrect HTTP200/timeouts/transport errors. Генератор пропустил 43 041 слот, не доказан 1000 RPS. Context native batch mean ~1,054 с остаётся узким местом.
- Полный pytest: 590 passed, zero skipped, 7 прежних предупреждений, 28,61 с; `artifacts/score-followup-tests.{log,xml}`. Реальные NER/NLI/Redis/100k проверки сохранены. Дополнительный false positive Bernard Shaw из прошлого этапа не исправлен. Все результаты/ограничения: `docs/score-followup.md`. Изменения только локальные; серверу нужен новый source и restart systemd. Сохранить `PII_NER_THREADS=4`; короткие автоматически используют 1. Torch/export не запускался.
